...
from capslayer import CapsuleLayer, PrimaryCap, Length, Mask
...
def base_cnn4(input_shape=(28, 28, 1), classes=10):
    model = Sequential()
    model.add(Conv2D(
        filters=32,
        kernel_size=(3, 3),
    ...
    model.add(BatchNormalization(axis=-1))
    # model.add(Conv2D(filters=128, kernel_size=(
    # 3, 3), padding="valid", activation='selu', kernel_initializer='lecun_normal'))
    model.add(Flatten())
